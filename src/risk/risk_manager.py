...
class RiskManager:
    ...
    def apply_rules(
        ...
        if self.use_trailing_stop and "close" in df.columns:
            ...
            if is_long.any():
                # We group by trade_blocks but only for rows where is_long is True
                # To do this safely, we mask the close price where not long
            ...
            # If a stop is triggered, all subsequent bars in the same contiguous block 
            # (which started before the stop) will also have stop_active = True
            stop_active = stop_triggered.groupby(trade_blocks).cumsum() > 0
            
            # Flat position on the day AFTER stop is hit
            adj_positions.loc[stop_active.shift(1).fillna(False)] = 0.0
